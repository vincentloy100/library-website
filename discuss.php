...
<?php
	...
	echo '<br><br><div style="text-align:center;"><h1>Discussion Room Booking List</h1>
			<br>
			...
			<input type="radio" name="sort" value="date">Booking Date&emsp;
			<input type="radio" name="sort" value="room">Room&emsp;
	...
	echo'<div class = "table">
			...
			<tr><br>
				<th class = "bla">Student ID</th>
				<th class = "bla">Student Name</th>
				<th class = "bla">Booking Date (mm/dd/yy) </th>
				<th class = "bla">Booking Time</th>
				<th class = "bla">Room</th>
				<th class = "bla">Checked</th>
			</tr>';
	if(isset($_POST['sortform'])){
		$sortt = $_POST['sort'];		
		if ($sortt=='id'){
			$query = "SELECT * FROM entries ORDER BY studentID";
		}
	
		else if ($sortt=='name'){
			$query = "SELECT * FROM entries ORDER BY studentName";
		}
	
		else if ($sortt=='date'){
			$query = "SELECT * FROM entries ORDER BY bookingDate";
		}
		
		else {
			$query = "SELECT * FROM entries ORDER BY room";
		}
		$r = mysqli_query($dbc, $query);
	
		while($row = mysqli_fetch_array($r)){
			echo "<tr>
				<td><br>{$row['studentID']}<br><br></td>
				<td><br>{$row['studentName']}<br><br></td>
				<td><br>{$row['bookingDate']}<br><br></td>
				<td><br>{$row['time']}<br><br></td>
				<td><br>{$row['room']}<br><br></td>
				<td><a href=\"check.php?id={$row['bookingID']}\"> <img src=\"images/ticked.png\" width=\"40\" height=\"40\"> </a></td>
				</tr>";		
		}
