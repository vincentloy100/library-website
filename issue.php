...
<!-------------------------------------------Content----------------------------------------------------->
	...
	<h1><br>Issue Books</h1><br>
	<form method="post">
		<p style = "text-align: center;">Book ID: <input type="text" name="bid" value = "<?php if (isset($_POST['issue'])) echo $_POST['bid']; ?>" required></p>
		<input type="submit" name="issue" value="Confirm">
		<br><br><br>
	</form>
<?php
	if (isset($_POST['issue'])){
		...
		if (count($errors) == 0){
			if (mysqli_num_rows($r)>0){
				...
				echo"<br><h6>Book Details:</h6><br>Book ID: {$row['id']}<br>Book Name: {$row['name']}<br>Book Author: {$row['author']}<br><br>
					<br><br><h6>Issue to?</h6><br>
					<form action=\"issue.php\" method=\"post\">
						Student Matriculation Number: <input type=\"text\" name=\"studid\" required>
						<input type = 'hidden' name = 'bid' value = '{$bookid}'>&emsp;
						<input type=\"submit\" name=\"cnfm\" value=\"Issue\">
					</form><br><br>";
			}
			else 
				array_push($errors, "Sorry, the record of the book ID entered isn't found. Please try again with another book ID.");
		}			
	}
	...
	if (isset($_POST['cnfm'])){
		...
		$a="SELECT * FROM user WHERE matno='$matno'";
		$result=mysqli_query($dbc, $a);
		if (mysqli_num_rows($result)==0){
			array_push($errors, "The student matriculation number entered is not yet registered.");
		}

		if (count($errors) == 0){
			$c = "INSERT INTO issue(studmat, bookid, date) VALUES('$matno', '$bookid', NOW())";
			@mysqli_query($dbc, $c);
			$d = "UPDATE books SET available = \"Not Available\" WHERE id = '$bookid'";
			@mysqli_query($dbc, $d);
			$e = "UPDATE books SET date = current_date() WHERE id = '$bookid'";
			@mysqli_query($dbc, $e);
			$_SESSION['issued']="The book with ID of ".$bookid." has been issued to ".$matno.".";
			mysqli_close($dbc);
		}				
	}
?>
	<?php include_once('signuperrors.php'); ?>
	<?php if (isset($_SESSION['issued'])): ?>
		<div class="success">
			<?php 
	...
	<?php endif ?>
		<br><a href = "issue.php"><b><i>Refresh</i></b></a><br>
